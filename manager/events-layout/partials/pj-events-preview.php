<?php
/**
 * PickleJar Live for Artists & Venues - Events Preview.
 *
...
$events_data_model = new PJEventsPageCallbacks();
$filters_query     = null;
if ( isset( $post ) ) {
	$events_layout_controller = new PJEventsPostTypeCallbacks();
	$the_post_id              = $post->ID;
	$post_data                = $events_layout_controller->get_post_meta_data( $the_post_id );
	$events_data              = $events_data_model->get_pj_event_layout_styles_data( $post_data );
	$filters_query            = $events_data_model->get_pj_event_filters_query( $post_data );
} else {
	$events_data = $events_data_model->get_pj_event_layout_styles_data();
}
	...
	<div class="picklejar-layout-preview">
		<div class="picklejar-row">
			...
			<div id="picklejar-layout-example-preview">
				<?php echo do_shortcode( '[pj_events_list ' . $shortcode_params . '"]' ); ?>
			</div>
		</div>
	</div>
</div>
